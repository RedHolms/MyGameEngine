...
public:
  ...
  constexpr List(List const& other) noexcept : List() {
    ...
    if constexpr (IsTriviallyCopyConstructibleType<T>) {
      Memory::Copy(m_buffer, other.m_buffer, m_count * sizeof(T));
    }
    else {
...
public:
  ...
  [[nodiscard]] constexpr Iterator at(size_t index) noexcept {
  ...
  [[nodiscard]] constexpr ConstIterator at(size_t index) const noexcept {
...
private:
  ...
  constexpr void _Reallocate(size_t newCapacity) noexcept {
    ...
    auto const newBuffer = Memory::Allocate(newCapacity * sizeof(T));
    ...
    if (m_count > 0) LIKELY {
      if constexpr (IsTriviallyCopyableType<T>) {
        Memory::Copy(newBuffer, m_buffer, m_count * sizeof(T));
      }
      else {
        for (size_t i = 0; i < m_count; ++i) {
